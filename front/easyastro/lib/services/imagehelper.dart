...
class ImageHelper {
  ...
  double stretch=0;
  double white=255; double black=0; double midtones = 128;double contrast = 1;
  ...
  void generateEncoded() {
    ...
    img.Color colorWhite = img.ColorUint8.rgba(white.toInt(),white.toInt(),white.toInt(),1);
    img.Color colorBlack = img.ColorUint8.rgba(black.toInt(),black.toInt(),black.toInt(),1);
    img.Color colorMid = img.ColorUint8.rgba(midtones.toInt(),midtones.toInt(),midtones.toInt(),1);

    img.Image image = original!.clone();
    ImageFilters.adjustColor(image, whites:colorWhite, blacks:colorBlack, mids:colorMid, contrast: contrast,
                                            rFactor: r, gFactor:g, bFactor:b);

    encoded = img.encodeJpg(image,quality:60);
    print("encoded");
  }
  ...
  void getParameters() async {
    apiHelper.get(ServerInfo().host, "/telescope/processing", queryParameters:{}).then((value) {
      ...
      midtones=(value["mids"]/256).clamp(0,255);
      print("$r $g $b $white $black $contrast $midtones");
    },);
  }
}