...
class ApiBaseHelper {
  ...
  Uri getUri(String host, String url, bool? ssl,
  ...
  Future<dynamic> get(String host, String url,
      {Map<String, dynamic>? queryParameters, bool? ssl, bool? binary}) async {
    dynamic responseJson;
    http.Response response;
    try {
      response = await http
          .get(getUri(host, url, ssl, queryParameters: queryParameters));
  ...
  Future<dynamic> post(String host, String url, dynamic body,
      {bool? ssl}) async {
    dynamic responseJson;
    try {
      final response = await http.post(getUri(host, url, ssl),
  ...
  Future<dynamic> put(String host, String url, dynamic body,
      {bool? ssl}) async {
    dynamic responseJson;
    try {
      final response =
          await http.put(getUri(host, url, ssl), body: jsonEncode(body));
      responseJson = _returnResponse(response);
    } catch (_) {
      lastError = 1;
      lastErrorStr = "no_connection";
